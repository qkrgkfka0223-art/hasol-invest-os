...
SCHEMA = "HASOL-WEB-CANDIDATES-v1"
ENGINE = "HASOL-WEB-CANDIDATE-QUANT-v1"
...
def run(input_path: Path, output_dir: Path) -> dict:
    ...
    if candidates.empty:
        bars = pd.DataFrame(columns=["ticker", "timestamp", "open", "high", "low", "close", "volume", "trade_count", "vwap"])
    ...
    (output_dir / "quant_top50.json").write_text(
        json.dumps(top50.to_dict(orient="records"), ensure_ascii=False, default=str, indent=2), encoding="utf-8"
    )
    (output_dir / "input_snapshot.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main() -> None:
    p = argparse.ArgumentParser(description="HASOL Web-first candidate Quant runner")
